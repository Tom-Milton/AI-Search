...
# python AlgAenhanced.py AISearchfile090a.txt

# Initialise variables
t0 = time.time()
tour = []
tour_length = float("inf")
termination_value = 10000
time_limit = 55
...
while time.time()-t0 < time_limit:
    ...
    """Generate nearest neighbour tour both ends"""
    # Initialise tour with random start city
    current_tour = [random.randrange(num_cities)]
    current_tour_length = 0

    # Generate remaining cities in tour
    while True:
        ...
        if len(current_tour) == num_cities:
            ...
            current_tour_length += dist_matrix[current_tour[0]][current_tour[-1]]
            break

        """Nearest neighbour both ends"""
        # Extracts row of dist_matrix for current start and end city of tour
        dist_array1 = dist_matrix[current_tour[0]].copy()
        dist_array2 = dist_matrix[current_tour[-1]].copy()

        # Makes already visited city distances infinity so they arent chosen as minimum
        for j in range(num_cities):
            if j in current_tour:
                dist_array1[j] = float('inf')
                dist_array2[j] = float('inf')

        # Chooses smallest distance from both ends
        minimums = [min(dist_array1), min(dist_array2)]
        minimum = min(minimums)
        n = minimums.index(minimum)
        # Adds the corresponding city to current tour
        if n == 0:
            current_tour.insert(0, dist_array1.index(minimum))
        else:
            current_tour.append(dist_array2.index(minimum))
        # Adds distance to tour length
        current_tour_length += minimum

    while time.time()-t0 < time_limit:
        """Performs 2opt swaps"""

        # Generates two points
        random_ints = random.sample(range(num_cities), k=2)
        point1 = min(random_ints)
        point2 = max(random_ints)

        # Reverses the section within the two points (2opt)
        new_tour = current_tour[:point1] + current_tour[point1:point2][::-1] + current_tour[point2:]

        # Calculates new tour length
        new_tour_length = 0
        for k in range(num_cities):
            if k == num_cities - 1:
                new_tour_length += dist_matrix[new_tour[0]][new_tour[-1]]
            else:
                new_tour_length += dist_matrix[new_tour[k]][new_tour[k + 1]]

        # Checks if new tour length is shorter
        if new_tour_length < current_tour_length:
            # Restarts counter
            c = 0
            current_tour, current_tour_length = new_tour, new_tour_length
        else:
            # Otherwise increase counter
            c += 1

        # If no changes for a given number of iterations
        if c > termination_value:
            # Compares the best tour found to the best overall
            if current_tour_length < tour_length:
                tour, tour_length = current_tour, current_tour_length
            break

# while time.time()-t0 < time_limit:
#
#     """Generate random nearest neighbour tour"""
#     # Initialise tour with random start city
#     current_tour = [random.randrange(num_cities)]
#     current_tour_length = 0
#
...
#         dist_array = dist_matrix[current_tour[-1]].copy()
#
#         # Makes already visited city distances infinity so they arent chosen as minimum
#         for j in range(num_cities):
#             if j in current_tour:
...
#
#         # Chooses three smallest distance
#         minimums = sorted(dist_array)[:2]
#         # Selects one randomly
#         if len(current_tour) == num_cities - 1:
#             n = random.randrange(1)
#         else:
#             n = random.randrange(2)
#         minimum = minimums[n]
#         # Adds the corresponding city to current tour
#         current_tour.append(dist_array.index(minimum))
#         # Adds distance to tour length
#         current_tour_length += minimum
#
#     change = True
#     # Runs until one cycle doesnt make any changes
#     while change:
#
#         # Forced termination
...
#             break
#
#         # Sets flag to show if a 2opt swap has been made to false
#         change = False
#
#         # Iterates through all pairs of edges
...
#                 # Checks if new tour length is shorter
#                 if new_tour_length < current_tour_length:
#                     # Increase counter to show a swap has been made
#                     change = True
#                     current_tour, current_tour_length = new_tour, new_tour_length
#
#     # Checks if current tour is better than the best found so far
#     if current_tour_length < tour_length:
#             tour, tour_length = current_tour, current_tour_length
