...
# python GeneticBasic.py AISearchfile048.txt
...
# Calculates initial minimum tour and stores it
tour, tour_length = min(population, key=lambda x: x[1])
...
while keyboard.is_pressed('q') is False:
    ...
    for i in range(int(population_size/2)):
        ...
        # # Ensures both children have no repetition of cities
        # for k1, v1 in enumerate(z1[:split]):
        #     if z1.count(v1) > 1:  # Checks if city occurs twice
        #         for k2, v2 in enumerate(z2):
        #             if v2 not in z1:  # Finds city not in z1
        #                 z1[k1], z2[k2] = v2, v1  # Swaps the two cities
        #                 break

        for z in [z1, z2]:
            # Swap two random cities if a mutation occurs
            if random.random() < mutation_chance:
                a, b = random.choices([i for i in range(num_cities)], k=2)
                z[a], z[b] = z[b], z[a]
            ...
            # If child is the new minimum, it is stored in memory
            if z_tour_length < tour_length:
                tour = z
                tour_length = z_tour_length
