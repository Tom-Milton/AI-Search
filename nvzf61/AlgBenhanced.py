...
# python AlgBenhanced.py AISearchfile090a.txt
...
for i in range(population_size):
    ...
    velocity = [(i, random.randrange(num_cities)) for i in range(num_cities)]
...
#
#     # Initialise tour with 0 start city
#     tour_i = [random.randint(0, num_cities)]
#     tour_length_i = 0
#
#     for j in range(num_cities-1):
#         # Extracts neighbouring cities for current end city of tour
#         dist_array = dist_matrix[tour_i[-1]].copy()
#
#         # Makes already visited city distances infinity so they arent chosen as minimum
#         for k, v in enumerate(dist_array):
#             if k in tour_i:
#                 dist_array[k] = float('inf')
#
#         # Chooses minimum distance and adds city to tour
...
while time.time()-t0 < 55:
    ...
    for z in range(population_size):
        ...
        for i in particle_velocity:
            particle_next_tour[i[0]], particle_next_tour[i[1]] = particle_next_tour[i[1]], particle_next_tour[i[0]]
        ...
        for i in range(num_cities):
            # Calculates current city in respective tours and its index in respective particle tours
            current_city1 = particle_next_tour[i]
            j1 = particle_tour_temp1.index(current_city1)
            current_city2 = particle_best_tour[i]
            j2 = particle_tour_temp2.index(current_city2)
            current_city3 = best_neighbour_tour[i]
            j3 = particle_tour_temp3.index(current_city3)

            # Normalises velocity
            if i != j1:
                particle_tour_temp1[i], particle_tour_temp1[j1] = particle_tour_temp1[j1], particle_tour_temp1[i]
                particle_velocity.append((i, j1))

            # Calculates swaps to get from particle_tour to particle_best_tour
            if i != j2:
                particle_tour_temp2[i], particle_tour_temp2[j2] = particle_tour_temp2[j2], particle_tour_temp2[i]
                best_tour_velocity.append((i, j2))

            # Calculates swaps to get from particle_tour to best_neighbour_tour
            if i != j3:
                particle_tour_temp3[i], particle_tour_temp3[j3] = particle_tour_temp3[j3], particle_tour_temp3[i]
                best_neighbour_velocity.append((i, j3))
        ...
        # # Alternative method to calculate particle next velocity
        # alpha, beta = random.random(), random.random()
        # new_best_tour_velocity = []
        # for i in best_tour_velocity:
        #     if random.random() < alpha:
        #         new_best_tour_velocity.append(i)
        # new_best_neighbour_velocity = []
        # for i in best_neighbour_velocity:
        #     if random.random() < beta:
        #         new_best_neighbour_velocity.append(i)
        # particle_next_velocity = particle_velocity + new_best_tour_velocity + new_best_neighbour_velocity

        # If a particle stops moving, give it a small random velocity to prevent convergence
        if len(particle_next_velocity) == 0:
            first_city = random.randrange(num_cities)
            second_city = random.randrange(first_city, num_cities)
            particle_next_velocity = [(first_city, second_city)]
